...
### 2.1. Core Logic (`content_script.js`)

- **Functionality:**
  - `getFilmaffinityTitle()`: Extracts the title from the page DOM.
  - `isFilmaffinitySeries()`: Determines content type (movie/series) by searching for specific text indicators within the page's structure relevant to series (e.g., "Serie de TV" or similar identifying markers).
  - `createTraktUrl(title)`: Builds the Trakt.tv search URL based on title and type.
  - `openTraktUrl(url)`: Attempts to open the constructed URL in a new tab using `chrome.tabs.create`. Includes error handling in case the tab cannot be opened.
- **DOM Interaction:**
  ...
  - Attaches event listeners (`mouseover`, `mouseout`, `click`) to the button.
    - Ensures the "Figtree" font is applied via `@font-face` definition in `styles/main.css` for
- **Error Handling:**
...
- **Content Scripts:**
  - Injects `content_script.js` and `styles/main.css`.
...
- **Implementation:**
  ...
  - `content_script.js` retrieves strings using `chrome.i18n.getMessage("keyName")`.
...
- **Improved Robustness:**
  - Make DOM element selection in `content_script.js` more resilient to potential changes in FilmAffinity's website structure.
